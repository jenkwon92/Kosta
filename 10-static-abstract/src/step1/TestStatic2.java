...
public class TestStatic2 {
	...
	public void test3() {
		test2(); //object 의 member method test3() 은 실행을 위해서는 객체 생성을 전제로 함으로 static member에 바로 접근 가능
		// test3() 이 실행된다는 것은 class loading-> 객체 생성을 전제로함으로 static은 미리 class loading 시점에 적재되므로, 
		// non-static 에서 static으로 바로 접근 가능
	}
	public static void main(String[] args) {
		System.out.println("main method는 static method이다");
		// compiler error : name은 instance variable 이므로 객체 생성시에만 메모리에 적재
		...
		t.test1();
		System.out.println(name2); //static member 이므로 바로 접근 가능
	}
}
