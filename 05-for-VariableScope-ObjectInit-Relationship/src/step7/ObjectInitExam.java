...
public class ObjectInitExam {
	 ...
	 * 새로운 객체의 주소값을 할당한다 
	 * 
	 * 3. pass1 메서드가 종료된다  (종료되면 stack영역에 f 라는 지역변수를 메모리에서 삭제되고
	 ...
	 * 이떄  setName() 의 인자값으로 쌀국수 문자열을 전달하는데
	 * 이는 메인에서 생성한 객체의 인스턴스 변수 정보를 수정하게 된다
	 * 즉, TestObjectInit2 의 main의 f 변수와 ObjectInitExam의 pass2의 f 변수는 
	 * 각각 다른 지역변수지만 동일한 갹체를 참조하고 있으므로  실제 Food 의 name이 
	 * 수정된다
	 */
