...
public class ConsoleInstUI6 {
	...
	// 구성원 추가 작업을 담당하는 메서드
	//addView() 메서드 기능 추가 
	//1. 입력 구성원 종류를 잘못 입력했을 때 다시 입력받게한다
	...
	public void addView() {
		...
		while(true) {
			...
			if(menu.equals("1")||menu.equals("2")||menu.equals("3")) { //정상입력일 경우 빠져나감
				break;
			}else {
		...
		//2번 추가 구현사항
		String tel = null;
		while(true) { //중복된 전화번호가 입력될 경우 메세지 출력 후 계속 반복해서 입력 
			...
			} catch (MemberNotFoundException e) { //catch구문 수행되면 tel은 중복된 번호가 아님
				break;
			}
		}
